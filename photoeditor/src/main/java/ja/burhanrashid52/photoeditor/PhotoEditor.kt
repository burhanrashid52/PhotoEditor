...
interface PhotoEditor {
     ...
     */
    @set:Deprecated("""use {@code setShape} of a ShapeBuilder
    ...
    @RequiresPermission(allOf = [Manifest.permission.WRITE_EXTERNAL_STORAGE])
    fun saveAsFile(imagePath: String,
     ...
     */
    @SuppressLint("StaticFieldLeak")
    fun saveAsBitmap(saveSettings: SaveSettings,
    ...
    class Builder(var context: Context, var parentView: PhotoEditorView) {
        var imageView: ImageView? = parentView.source
        var deleteView: View? = null
        var drawingView: DrawingView? = parentView.drawingView
