 ...
 *
 */
internal class MultiTouchListener(deleteView: View?,
    ...
    companion object {
        ...
        private fun adjustAngle(degrees: Float): Float {
            var degreesSet = degrees
            if (degreesSet > 180.0f) {
                degreesSet -= 360.0f
            } else if (degreesSet < -180.0f) {
                degreesSet += 360.0f
            }
            return degreesSet
        }
        ...
        private fun move(view: View, info: TransformInfo) {
            ...
            scale = info.minimumScale.coerceAtLeast(info.maximumScale.coerceAtMost(scale))
            view.scaleX = scale
            view.scaleY = scale
